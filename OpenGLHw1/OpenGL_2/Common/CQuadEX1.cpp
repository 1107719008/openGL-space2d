...
{	
	//enmey point shape
	_nVtx = QUAD_NUM_ONE;
	_Points = new vec4[QUAD_NUM_ONE];
	_Points[0] = vec4(-1.0f, 2.0f, 0.0f, 1.0f);
	_Points[1] = vec4(-1.0f, 3.0f, 0.0f, 1.0f);
	_Points[2] = vec4(-0.0f, 2.0f, 0.0f, 1.0f);
	_Points[3] = vec4(0.0f, 2.0f, 0.0f, 1.0f);
	_Points[4] = vec4(1.0f, 3.0f, 0.0f, 1.0f);
	_Points[5] = vec4(1.0f, 2.0f, 0.0f, 1.0f);
	_Points[6] = vec4(1.0f, -1.0f, 0.0f, 1.0f);
	_Points[7] = vec4(-1.0f, -1.0f, 0.0f, 1.0f);

	_Colors = new vec4[QUAD_NUM_ONE];
...
void CQuadEX1::draw()
{
	...
	glDrawArrays( GL_TRIANGLES, 0, QUAD_NUM_ONE);
}
...
void CQuadEX1::Move(float dt) 
{
	mat4 movingT;
	
	fEnemyY -= MOVESPEED *dt;
	if (fEnemyY<-17)
	{
		fEnemyY = 15;
	}
...
void CQuadEX1::Disable(float dt)
{
	mat4 disT, moveT;
	fScale -= dt;
